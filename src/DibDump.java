...
public class DibDump
{
    ...
    *
    */
   public void makePels(String inputFile, String outputFile)
   {
...
*/
         ...
         System.out.printf("bfType=%2X bfSize=%d bfReserved1=%h bfReserved2=%h bfOffBits=%d\n", bmpFileHeader_bfType, bmpFileHeader_bfSize, bmpFileHeader_bfReserved1, bmpFileHeader_bfReserved2, bmpFileHeader_bfOffBits);
...
*/
         ...
         System.out.printf("biSize=%d\nbiWidth=%d\nbiHeight=%d\nbiPlanes=%d\nbiBitCount=%d\nbiCompression=%d\nbiSizeImage=%d\nbiXPelsPerMeter=%d\nbiYPelsPerMeter=%d\nbiClrUsed=%d\nbiClrImportant=%d\n", bmpInfoHeader_biSize, bmpInfoHeader_biWidth, bmpInfoHeader_biHeight, bmpInfoHeader_biPlanes, bmpInfoHeader_biBitCount, bmpInfoHeader_biCompression, bmpInfoHeader_biSizeImage, bmpInfoHeader_biXPelsPerMeter, bmpInfoHeader_biYPelsPerMeter, bmpInfoHeader_biClrUsed, bmpInfoHeader_biClrImportant);
...
*/
      ...
      try{
         double [][]scaled = new double[imageArray.length][imageArray[0].length];
         BufferedWriter out = new BufferedWriter(new FileWriter(outFileName, true));
         for(int ind = 0; ind < bmpInfoHeader_biHeight; ind++){
            for(int ind2 = 0; ind2 < bmpInfoHeader_biWidth; ind2++){
               out.write(" ");
               out.write(new Double((imageArray[ind][ind2]  & 0x00FFFFFF)/16777216.0).toString());
            }
            System.out.println();
         }
         out.newLine();
   ...
   } // public void makePels

   public void makeMap(String inFile, String outFile){
      String inFileName, outFileName;
      int i, j, k;
      int numberOfColors;
      int pel;
      ...
      try
      {
         ...
         for(int ind1 = 0; ind1 < bmpInfoHeader_biHeight; ind1++){
            for(int ind2 = 0; ind2 < bmpInfoHeader_biWidth; ind2++ ){
               double val = scan.nextDouble();
               imageArray[ind1][ind2] =  (int) (val*16777216);
            }
         }
